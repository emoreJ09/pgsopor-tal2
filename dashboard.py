...
class OfficeDashboard(tk.Frame):
    ...
    def open_edit_pow_modal(self):
        ...
        if not hasattr(self, 'proj_tree') or not self.proj_tree.selection():
            messagebox.showwarning("Walang Napili", "Hindi mahanap ang aktibong selection table para sa edit action.")
            return
        ...
        current_location = self.proj_tree.item(selected_item, 'tags')[0] if self.proj_tree.item(selected_item, 'tags') else ""
        ...
        def save_all_updates_to_db():
            ...
            if success_main and success_items:
                ...
                edit_win.destroy() 
                if hasattr(self, 'load_projects_from_db'):
                    self.load_projects_from_db() 
            else:
                messagebox.showerror("SQL Error", "May error sa database query. Pakisuri kung tugma ang table names.")
